...
namespace SwaySettings {
    ...
    public class Functions {
        ...
        public static void write_settings (string file_name, Array<string> lines) {
            try {
                ...
                var fos = file.replace (null,
                                        ...
                                        null);
                var dos = new DataOutputStream (fos);
                dos.put_string ("# GENERATED BY SWAY-SETTINGS. DON'T MODIFY THIS FILE!\n");
        ...
        public static File check_program_settings_folder_exists () {
            string basePath = GLib.Environment.get_user_config_dir () +
                              "/" + Strings.program_settings_folder_name;
            // Checks if directory exists. Creates one if none
            if (!GLib.FileUtils.test (basePath, GLib.FileTest.IS_DIR)) {
                try {
                    var folder = File.new_for_path (basePath);
            ...
            // Checks if file exists. Creates one if none
            var file = File.new_for_path (basePath + @"/$(Strings.program_settings_config_name)");
            if (!file.query_exists ()) {
                try {
                    file.create (FileCreateFlags.NONE);
                } catch (Error e) {
        ...
        public static ArrayList<Keybinding> get_keybindings () {
            var keybindings = new ArrayList<Keybinding>();
            string path = GLib.Environment.get_user_config_dir () +
                          "/" + Strings.program_settings_folder_name +
                          "/" + Strings.program_settings_config_name;
            var parser = new Json.Parser ();
            try {
                parser.load_from_file (path);
                var root = parser.get_root ();
                var shortcuts = root.get_object ().get_object_member ("shortcuts");
                shortcuts.foreach_member ((obj, name, node) => {
                    var object = node.get_object ();
                    if(object == null) return;
                    keybindings.add(new Keybinding (name, object));
                });
            } catch (Error e) {
                print ("Error: %s\n", e.message);
                Process.exit (1);
        ...
        public static ArrayList<Keybinding_Variable> get_keybinding_variables () {
            var keybindings = new ArrayList<Keybinding_Variable>();
            string path = GLib.Environment.get_user_config_dir () +
                          "/" + Strings.program_settings_folder_name +
                          "/" + Strings.program_settings_config_name;
            var parser = new Json.Parser ();
            try {
                parser.load_from_file (path);
                var root = parser.get_root ();
                var shortcuts = root.get_object ().get_object_member ("shortcut-vars");
                shortcuts.foreach_member ((obj, name, node) => {
                    var object = node.get_string ();
                    if(object == null) return;
                    keybindings.add(new Keybinding_Variable (name, object));
                });
            } catch (Error e) {
                print ("Error: %s\n", e.message);
                Process.exit (1);
